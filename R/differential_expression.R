...
run_deseq2 <- function(count_matrix,
  ...
  message("DESeq2: ", sum(res_df$padj < padj_threshold & !is.na(res_df$padj)),
...
run_edger <- function(count_matrix,
  ...
  message("edgeR: ", sum(res$padj < padj_threshold & !is.na(res$padj)),
...
run_limma_voom <- function(count_matrix,
  ...
  message("limma-voom: ", sum(res$padj < padj_threshold & !is.na(res$padj)),
...
consensus_de <- function(count_matrix,
  ...
  if ("deseq2" %in% methods) {
    all_results$deseq2 <- tryCatch(
      ...
      error = function(e) {
        message("DESeq2 failed: ", e$message); NULL
      }
    )
  }
  ...
  if ("edger" %in% methods) {
    all_results$edger <- tryCatch(
      ...
      error = function(e) {
        message("edgeR failed: ", e$message); NULL
      }
    )
  }
  ...
  if ("limma" %in% methods) {
    all_results$limma <- tryCatch(
      ...
      error = function(e) {
        message("limma failed: ", e$message); NULL
      }
    )
  }
  ...
  rank_cols <- lapply(names(all_results), function(m) {
    res <- all_results[[m]]
    res_df <- data.frame(gene = res$gene, rank = seq_len(nrow(res)),
                         stringsAsFactors = FALSE)
    names(res_df)[2] <- paste0("rank_", m)
    res_df
  })

  consensus <- Reduce(function(x, y) merge(x, y, by = "gene", all.x = TRUE),
  ...
  message("Consensus DE: ", nrow(top_de), " genes detected by >= ",
          min_methods, " methods")
